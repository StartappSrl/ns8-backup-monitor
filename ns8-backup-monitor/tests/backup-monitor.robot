...
*** Test Cases ***
Check if backup-monitor is installed correctly
...
Check if backup-monitor can be configured
    ${rc} =    Execute Command    api-cli run module/${module_id}/configure-module --data '{}'
    ...    return_rc=True  return_stdout=False
    Should Be Equal As Integers    ${rc}  0

Check if backup-monitor works as expected
    ${rc} =    Execute Command    curl -f http://127.0.0.1/backup-monitor/
    ...    return_rc=True  return_stdout=False
    Should Be Equal As Integers    ${rc}  0

Check if backup-monitor is removed correctly
